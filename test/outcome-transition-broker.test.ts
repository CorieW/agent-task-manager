...
import { InMemoryProvider, OutcomeTransitionBroker, type ProviderEnvironment, type SubAgentDefinition, type WorkspaceSchemaDescriptor } from "../src/index.js";
...
test("requires and persists human recovery for an explicitly declared outcome", async () => {
  ...
  assert.match((await provider.getTaskSnapshot("task-1")).body, /agent-task-manager:human-slot/u);
});
...
function providerWithTask(): InMemoryProvider {
  ...
  provider.seedTaskStatusOptions(["Coding", "Needs Human Resolution", "Review"]);
  provider.seedTask({ archived: false, body: "Task body", dependencies: [], id: "task-1", priority: 1, properties: { Status: "Coding" }, status: "Coding", title: "Task", version: "v1" });
  return provider;
}
