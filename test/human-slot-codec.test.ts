...
test("round-trips every canonical human interaction kind", () => {
  for (const kind of ["answer", "resolution", "review", "testing"] as const) {
    /** Represents the canonical human-interaction slot under test. */
    const slot = createHumanInteractionSlot({
      createdAt: "2026-08-15T10:00:00.000Z",
      generation: 1,
      kind,
      prompt: `Respond to ${kind}`,
      requestedBy: "manager",
      routes: { approve: "Done", return: "Todo" },
      sourceErrorKey: kind === "resolution" ? "error/one" : null,
      taskId: "task-1",
    });
    assert.deepEqual(
      parseHumanInteractionSlots(renderHumanInteractionSlot(slot)),
      [slot],
    );
    assert.equal(
      parseHumanInteractionSlots(
        appendHumanInteractionSlot("Task context", slot),
      ).length,
      1,
    );
  }
...
test("accepts one response while rejecting machine-owned edits", () => {
  /** Preserves the machine-owned Task state before human edits. */
  ...
  assert.equal(
    verifyAllowedHumanDelta(baseline, edited).targetStatus,
    "Testing",
  );
  assert.throws(
    () => verifyAllowedHumanDelta(baseline, { ...edited, prompt: "Changed" }),
    /digest|machine-owned/,
  );
  assert.throws(
    () =>
      verifyAllowedHumanDelta(baseline, {
        ...edited,
        response: { action: "delete", text: "No" },
      }),
    /not allowed/,
  );
  assert.throws(
    () =>
      verifyAllowedHumanDelta(baseline, {
        ...edited,
        response: { action: "constructor", text: "No" },
      }),
    /not allowed/,
  );
});
...
test("rejects malformed and duplicate slot markers", () => {
  ...
  const slot = createHumanInteractionSlot({
    ...
    prompt: "Answer",
    requestedBy: "planner",
    routes: { submit: "Planning" },
    sourceErrorKey: null,
    taskId: "task-1",
  });
  /** Carries the rendered slot text corrupted by the scenario. */
  ...
  assert.throws(
    () =>
      parseHumanInteractionSlots(rendered.replace(":end -->", ":broken -->")),
    /malformed/,
  );
});
