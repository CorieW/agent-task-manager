...
export function resolveExternalEffectEnvironment(config: EnvironmentConfig, installed: readonly ExternalEffectHandler[]): ResolvedExternalEffectEnvironment {
  const byIdentity = new Map(installed.map((handler) => [`${handler.kind}\0${handler.id}`, handler]));
  if (byIdentity.size !== installed.length) throw new Error("Installed external-effect handlers must have unique kind and adapter identities");
  const handlers = new ExternalEffectHandlerRegistry();
  for (const [kind, adapterId] of Object.entries(config.effects.handlers).sort(([left], [right]) => left.localeCompare(right))) {
    const handler = byIdentity.get(`${kind}\0${adapterId}`);
    if (handler === undefined) throw new Error(`Configured external-effect handler is unavailable: ${kind} -> ${adapterId}`);
