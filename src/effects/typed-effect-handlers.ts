...
import type { ExternalEffectHandler, ExternalEffectObservation, ExternalEffectRequest } from "./contracts.js";
...
export interface WorkspaceReleasePayload { readonly workspaceKey: string; }
...
export interface CommandRunPayload { readonly arguments: readonly string[]; readonly commandKey: string; readonly workspaceKey: string; }
export interface BrowserRunPayload { readonly environmentKey: string; readonly scenarioResource: string; readonly workspaceKey: string; }
export interface ChildAgentNode { readonly contextResource: string; readonly definitionId: string; readonly dependsOn: readonly string[]; readonly nodeKey: string; }
...
export interface ReconcilableEffectAdapter<T> {
  ...
  apply(input: { readonly effectId: string; readonly payload: T }): Promise<ExternalEffectObservation>;
  reconcile(input: { readonly effectId: string; readonly payload: T }): Promise<ExternalEffectObservation>;
}
...
function handler<T>(kind: ExternalEffectKind, adapter: ReconcilableEffectAdapter<T>, parse: (payload: JsonObject) => T): ExternalEffectHandler {
  ...
  return {
    ...
    apply: (request: ExternalEffectRequest) => adapter.apply({ effectId: request.effectId, payload: parse(request.payload) }),
    reconcile: (request: ExternalEffectRequest) => adapter.reconcile({ effectId: request.effectId, payload: parse(request.payload) }),
...
function parseWorkspaceRelease(value: JsonObject): WorkspaceReleasePayload { exact(value, ["workspaceKey"]); return { workspaceKey: key(value.workspaceKey, "workspaceKey") }; }
...
function parseCommandRun(value: JsonObject): CommandRunPayload { exact(value, ["arguments", "commandKey", "workspaceKey"]); const args = stringList(value.arguments, "arguments"); if (args.length > 100 || args.some((entry) => entry.length > 10_000)) throw new TypeError("command.run arguments exceed their bounds"); return { arguments: args, commandKey: key(value.commandKey, "commandKey"), workspaceKey: key(value.workspaceKey, "workspaceKey") }; }
function parseBrowserRun(value: JsonObject): BrowserRunPayload { exact(value, ["environmentKey", "scenarioResource", "workspaceKey"]); return { environmentKey: key(value.environmentKey, "environmentKey"), scenarioResource: key(value.scenarioResource, "scenarioResource"), workspaceKey: key(value.workspaceKey, "workspaceKey") }; }
function parseChildAgentWave(value: JsonObject): ChildAgentWavePayload {
  ...
  const nodes = value.nodes.map((entry, index) => {
    const node = object(entry, `nodes[${index}]`); exact(node, ["contextResource", "definitionId", "dependsOn", "nodeKey"]);
    return { contextResource: key(node.contextResource, "contextResource"), definitionId: text(node.definitionId, "definitionId"), dependsOn: stringList(node.dependsOn, "dependsOn").map((item) => key(item, "dependency")), nodeKey: key(node.nodeKey, "nodeKey") };
  });
