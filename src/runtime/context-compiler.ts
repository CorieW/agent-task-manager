...
export async function compileRunContext(input: {
  ...
  if (bytes > resolved.definition.contextBudgetBytes) throw new Error("Compiled run context exceeds the Sub-agent context budget");
  return finalizeRunContext(core);
}

function requiredString(value: string, label: string): string { if (value === "") throw new TypeError(`${label} is required`); return value; }
