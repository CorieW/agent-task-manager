...
Workspace validation accepts either Notion relation mode when the provider owns only the forward relation, such as Task `Dependencies`; an existing unused reciprocal property is compatible. Relations whose reciprocal property is provider-owned, such as Task `Active Agents`, still require the exact synchronized property name.
